  ...
  module Parser
    describe Parser do
      ...
      it 'parses basic variable declarations' do
        expect(parser.var_decl).to parse "aa : BOOL;", trace: true
        expect(parser.var_decl).to parse "aa:BOOL ;", trace: true
        expect(parser.var_decl).to parse "bb : INT;", trace: true
        expect(parser.var_decl).to parse "cc : WORD;", trace: true
        expect(parser.var_decl).to parse "o : STRING   [254 ]   :=     'awltool rocks $$ $' $l $L $p $P $r $R $t $T';", trace: true
      end
      ...
      it 'parses date and time' do
        ...
        expect(parser.time_value).to parse "T#5D6H7M8S100MS", trace: true
        expect(parser.value).to parse "D#1990-1-1" # DATE
        expect(parser.value).to parse "TOD#8:33:22.111", trace: true # TIME_OF_DAY
        expect(parser.value).to parse "DT#90-1-1-8:33:22.111" # DATE_AND_TIME
      end
      ...
      it 'parses chars' do
        # note the parser does not differentiate between satrings and chars
